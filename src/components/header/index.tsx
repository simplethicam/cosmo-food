...
import { IOrder, IStore, ICourier, IIdentity, IStaff } from "../../interfaces";
...
export const Header: React.FC<RefineThemedLayoutV2HeaderProps> = () => {
  ...
  const { refetch: refetchStaff } = useList<IStaff>({
    ...
    queryOptions: {
      enabled: false,
      onSuccess: (data) => {
        const staffOptionGroup = data.data.map((item) => {
          return {
            label: `${item.name}`,
            avatar: (
              <Avatar
            ...
            link: `/staff/edit/${item.id}`,
            category: t("staff.staff"),
          };
        });
        setOptions((prevOptions) => [...prevOptions, ...staffOptionGroup]);
      },
    },
  });*/
